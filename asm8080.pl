...
#   errFlag:      Boolean;     { TRUE if error occurred this line }
	$errCount = 0;     # Total number of errors
#
#   line:         String( 256 );      { Current line from input file }
#   listLine:     String( 256 );      { Current listing line }
...
#
#   bytStr:       String( 256 );      { Buffer for long DB statements }
...
#   cl_SrcName:   String( 256 );      { Source file name }
#   cl_ListName:  String( 256 );      { Listing file name }
#   cl_ObjName:   String( 256 );      { Object file name }
	$cl_Err = 1;	# TRUE for errors to screen
#
#
#   source:       Text;
#   object:       Text;
...
#
#
#FUNCTION Deblank(s: String): Str256;
#
#VAR
#   i: Integer;
#
...
#
#
#FUNCTION UprCase(s: String): Str256;
#
#VAR
#   i: Integer;
#
...
#
#
#FUNCTION Hex2(i: Integer): Str256;
#
#BEGIN
#   i := i AND 255;
...
#
#
#FUNCTION Hex4(i: Integer): Str256;
#
#BEGIN
...
#
#
#PROCEDURE Error(message: Str256);
#
#BEGIN
#   errFlag := TRUE;
#   Inc(errCount);
...
sub InitOpcodes {
	...
	$opcdTab{'NOP'} = [ o_None, 0 ];
	$opcdTab{'RLC'} = [ o_None, 7 ];
	$opcdTab{'RRC' } = [ o_None, 15 ];
	$opcdTab{'RAL' } = [ o_None, 23 ];
	$opcdTab{'RAR' } = [ o_None, 31 ];
	$opcdTab{'DAA' } = [ o_None, 39 ];
	$opcdTab{'CMA' } = [ o_None, 47 ];
	$opcdTab{'RIM' } = [ o_None, 48 ];
	$opcdTab{'STC' } = [ o_None, 55 ];
	$opcdTab{'SIM' } = [ o_None, 56 ];
	$opcdTab{'CMC' } = [ o_None, 63 ];
	$opcdTab{'HLT' } = [ o_None, 118 ];
	$opcdTab{'RNZ' } = [ o_None, 192 ];
	$opcdTab{'RZ'  } = [ o_None, 200 ];
	$opcdTab{'RET' } = [ o_None, 201 ];
	$opcdTab{'RNC' } = [ o_None, 208 ];
	$opcdTab{'RC'  } = [ o_None, 216 ];
	$opcdTab{'RPO' } = [ o_None, 224 ];
	$opcdTab{'XTHL'} = [ o_None, 227 ];
	$opcdTab{'RPE' } = [ o_None, 232 ];
	$opcdTab{'PCHL'} = [ o_None, 233 ];
	$opcdTab{'XCHG'} = [ o_None, 235 ];
	$opcdTab{'RP'  } = [ o_None, 240 ];
	$opcdTab{'DI'  } = [ o_None, 243 ];
	$opcdTab{'RM'  } = [ o_None, 248 ];
	$opcdTab{'SPHL'} = [ o_None, 249 ];
	$opcdTab{'EI'  } = [ o_None, 251 ];

	$opcdTab{'ADI' } = [ o_One, 198 ];
	$opcdTab{'ACI' } = [ o_One, 206 ];
	$opcdTab{'OUT' } = [ o_One, 211 ];
	$opcdTab{'SUI' } = [ o_One, 214 ];
	$opcdTab{'IN'  } = [ o_One, 219 ];
	$opcdTab{'SBI' } = [ o_One, 222 ];
	$opcdTab{'ANI' } = [ o_One, 230 ];
	$opcdTab{'XRI' } = [ o_One, 238 ];
	$opcdTab{'ORI' } = [ o_One, 246 ];
	$opcdTab{'CPI' } = [ o_One, 254 ];

	$opcdTab{'SHLD'} = [ o_Two, 34 ];
	$opcdTab{'LHLD'} = [ o_Two, 42 ];
	$opcdTab{'STA' } = [ o_Two, 50 ];
	$opcdTab{'LDA' } = [ o_Two, 58 ];
	$opcdTab{'JNZ' } = [ o_Two, 194 ];
	$opcdTab{'JMP' } = [ o_Two, 195 ];
	$opcdTab{'CNZ' } = [ o_Two, 196 ];
	$opcdTab{'JZ'  } = [ o_Two, 202 ];
	$opcdTab{'CZ'  } = [ o_Two, 204 ];
	$opcdTab{'CALL'} = [ o_Two, 205 ];
	$opcdTab{'JNC' } = [ o_Two, 210 ];
	$opcdTab{'CNC' } = [ o_Two, 212 ];
	$opcdTab{'JC'  } = [ o_Two, 218 ];
	$opcdTab{'CC'  } = [ o_Two, 220 ];
	$opcdTab{'JPO' } = [ o_Two, 226 ];
	$opcdTab{'CPO' } = [ o_Two, 228 ];
	$opcdTab{'JPE' } = [ o_Two, 234 ];
	$opcdTab{'CPE' } = [ o_Two, 236 ];
	$opcdTab{'JP'  } = [ o_Two, 242 ];
	$opcdTab{'CP'  } = [ o_Two, 244 ];
	$opcdTab{'JM'  } = [ o_Two, 250 ];
	$opcdTab{'CM'  } = [ o_Two, 252 ];

	$opcdTab{'INR' } = [ o_InrDcr, 4 ];
	$opcdTab{'DCR' } = [ o_InrDcr, 5 ];

	$opcdTab{'ADD' } = [ o_Arith, 128 ];
	$opcdTab{'ADC' } = [ o_Arith, 136 ];
	$opcdTab{'SUB' } = [ o_Arith, 144 ];
	$opcdTab{'SBB' } = [ o_Arith, 152 ];
	$opcdTab{'ANA' } = [ o_Arith, 160 ];
	$opcdTab{'XRA' } = [ o_Arith, 168 ];
	$opcdTab{'ORA' } = [ o_Arith, 176 ];
	$opcdTab{'CMP' } = [ o_Arith, 184 ];

	$opcdTab{'MOV' } = [ o_MOV, 64 ];

	$opcdTab{'MVI' } = [ o_MVI, 6 ];

	$opcdTab{'LXI' } = [ o_LXI, 1 ];

	$opcdTab{'INX' } = [ o_InxDcx, 3 ];
	$opcdTab{'DAD' } = [ o_InxDcx, 9 ];
	$opcdTab{'DCX' } = [ o_InxDcx, 11 ];

	$opcdTab{'POP' } = [ o_PushPop, 193 ];
	$opcdTab{'PUSH'} = [ o_PushPop, 197 ];

	$opcdTab{'STAX'} = [ o_StaxLdax, 2 ];
	$opcdTab{'LDAX'} = [ o_StaxLdax, 10 ];

	$opcdTab{'RST' } = [ o_RST, 199 ];
	...
	$opcdTab{'ORG' } = [ o_ORG, 0 ];
	$opcdTab{'END' } = [ o_END, 0 ];
	$opcdTab{'LIST'} = [ o_LIST, 0 ];
	$opcdTab{'OPT' } = [ o_OPT, 0 ];
 
	if( LevelCheck( 1, $Proc ) and $DBG ) {
		print Dumper( %opcdTab );
	}
...
#
#
#PROCEDURE Expect(expected: Str256);
#
#BEGIN
...
#
#
#FUNCTION EvalOct(octStr: Str256): Integer;
#
#VAR
#   octVal:  Integer;
#   evalErr: Boolean;
...
#
#
#FUNCTION EvalDec(decStr: Str256): Integer;
#
#VAR
#   decVal:  Integer;
#   evalErr: Boolean;
...
#
#
#FUNCTION EvalHex(hexStr: Str256): Integer;
#
#VAR
#   hexVal:  Integer;
#   evalErr: Boolean;
...
#
#
#FUNCTION FindReg(regName,regList,valList: Str256): Integer;
#
#VAR
#   p:    Integer;
#   reg:  Integer;
...
#################################################################
# Title:	DoPass
# Function:	Do a pass thru the source file.
#
sub DoPass {
	my $Proc = "DoPass";
    my $DBG = 1;
...
#   CodeOrg(0);
	...
	open( SRC, "<$cl_SrcName" ) || die "Can't open ASM file: $cl_SrcName";
	while( <SRC> ) {
...
if( length( $line ) > 0 ) {
	...
	if( m/^\w/ ) {
		print "Itsa label\n";
		( $labl, $line ) = leftright( $line );
		if( LevelCheck( 1, $Proc ) and $DBG ) {
			print "labl: \"$labl\"     line: \"$line\"\n";
		}
	}
}
...
#
#
#FUNCTION GetOption(VAR optStr: Str256): Str256;
#
#VAR
#   option: String[80];
#   p:      Integer;
...
#
#
#FUNCTION GetOptions(VAR cl_SrcName, cl_ListName,cl_ObjName: Str256;
...
#   Rewrite(object);
	...
	if( LevelCheck( 2, $Proc ) and $DBG ) {
		print "Asm File: $cl_SrcName\n";
		print "List File: $cl_ListName\n";
		print "Obj File: $cl_ObjName\n";
	}
#
#   symTab    := NIL;
	$xferAddr  = 0;
